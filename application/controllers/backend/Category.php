...
class Category extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // $rule_category = $this->db->get()->result();
        ...
        if (privilage($this->user, "category", "MAdd")) {
            $bt_add = 1;
        } else {
            $bt_add = 0;
        }
        ...
        // $data_content['rule_category']   = $rule_category;
        ...
        /* TEMPLATE */
        $config["content_file"] = "tr_point/category";
    ...
    public function get_list()
    {
        ...
        $urlDl = "'" . base_url("backend/category/delete") . "'";
        $path = "upload/BILL/";
        ...
        foreach ($all_data as $key => $val) {
            ...
            $row[]    = $number;
            $row[]    = $val->MemberID;
            $row[]    = '<text-align: right;>' . number_format($val->TRXAmount, 2, ".", ",");
            $row[]    = $val->Point;
            if ($val->TRXPhoto) {
            ...
            if (privilage($this->user, "category", "MDelete")) {
    ...
    public function detail_category()
    {
        ...
        $this->db->select("rcd.*, rc.RuleTitle, rc.StartDate");
        $this->db->from("rule_category_detail rcd");
        $this->db->join("rule_category rc", "rcd.PDID = rc.DID ", "LEFT");
        $this->db->where("rc.StartDate > now()");
        $this->db->where("(rc.RuleTitle LIKE '%" . $this->input->post("search")['value'] . "%' 
                           or rcd.RuleCategory LIKE '%" . $this->input->post("search")['value'] . "%'
                        )");
        if ($this->input->post("length") != -1)
            $this->db->limit($this->input->post("length"), $this->input->post("start"));
        $all_data = $this->db->get()->result();
        ...
        foreach ($all_data as $key => $val) {
            ...
            // $row[]    = $val->StartDate;
            $row[]    = '<center><button type="button" class="btn btn-primary btn-sm" onclick="setcategory(this)" value="' . $val->RuleTitle ." - ". $val->RuleCategory. "/" . $val->PDID . "," . $val->PointRatio . "," . $val->EventMultiply . '">select</button></center>';
    ...
    public function add_point()
    {
        ...
        $split = explode(",",$this->input->post("titlecategory"));
        $rule_id = $split[0];
        ...
        $floor = floor((int) preg_replace("/[^0-9]/", "", substr($_POST["trxamount"], 0, -3)) / (int) $split[1]); //ratio
        $point = (int) $floor * (int) $split[2]; //eventmutiply
        ...
        $data_insert["RuleType"]      = "CATEGORY";
        $data_insert["RuleID"]        = $rule_id ;
        ...
        $data_insert["ExpiredStatus"] = 0; // no expired
